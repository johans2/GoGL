...
func (r *renderer) init(verts []float32, shader uint32) {
	r.verts = verts
	r.shader = shader
	...
	normalAttrib := uint32(gl.GetAttribLocation(shader, gl.Str("normal\x00")))
...
func (r *renderer) issueDrawCall(texture uint32, MVP mgl32.Mat4) {
	...
	MVPuniform := gl.GetUniformLocation(r.shader, gl.Str("MVP\x00"))
